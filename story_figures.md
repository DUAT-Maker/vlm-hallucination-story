...
## 正文图 2：层段论证——读出在哪、流失在哪、哪里注入安全

![band_evidence](assets/svd/band_evidence.png)

左：视觉读出（$IL_{vis}$）的逐层分布与逐层流失量——**83.7% 的视觉读出、85.9% 的流失都集中在 L5-26**（阴影带），L0-4 无可救、L27-31 只剩尾部；右：注入安全性（500 图）——**装配层（L27-31）单独注入反而恶化幻觉**（55.8 > greedy 49.4）。

**层段 L5-26 的完整论证**：视觉读出集中带 ∩ 流失集中带 ∩ 注入安全带——三个独立测量指向同一个区间。下界由"无流失可救"截断（现象），上界由"装配层注入有害"截断（机制实验）。
