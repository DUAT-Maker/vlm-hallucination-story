...
## 正文图 2：层段论证（层段是测出来 + 比出来的，不是拍的）

![band_justification](assets/svd/band_justification.png)

位置匹配的逐层可分性（$IL_{vis}$ 的 Cohen's d，即逐层标准化）：**L0-4 无病灶信号（d≈+0.06）、L5 起持续可读（band 均值 +0.30）**。注：证据带内 $A_{img}$ 的 d 为负（幻觉实体看得更多却读到更少，误绑定签名），文字带过不进图。

**层段的承重金属是 500 图消融表**（非本图）：L5-18（HGAI 论文段）36.0/8.1 → L14-26（只用证据带）53.8（早段搬运用力不足）→ L0-31（全层）更差 → **L5-26 = 32.0/6.9/73.1 最优**。本图只负责回答"为什么这些层参与干预"，层段边界由消融定。
