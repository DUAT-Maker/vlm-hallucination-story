...
位置匹配的逐层可分性（Cohen's d）：内容通路（$IL_{vis}$/$VS$）从 L5 起持续可分，L0-4 无信号；证据带内 $A_{img}$ 的 d 为**负**——幻觉实体"看得更多、读到更少"（误绑定签名）。层段 L5-26 由此图谱推导，并经消融验证（L5-18 / L14-26 / L0-31 均更差）。**干预只施加在病灶可读的层段。**
