...
## 33. floor 层响应度与动态剂量证伪（2026-09-03）

**响应度剖面**（$s_{I,t}^{(l)}=(1-m_I^{(l)})\sum_jA_j^{(l)}F_j^{(l)}$，floor 在 β=0 处对图像质量的边际作用；20 图 4 分片，`tools/logit_decomp/floor_responsiveness.py`，图 `assets/svd/floor_responsiveness.png`）：

- 剖面呈 band 形：L0-4 = 10.8%、**L5-26 = 74.5%**、L27-31 = 14.6%（峰在 L20）——**层段第三条独立证据**（与 drain 剖面 §31、logit lens §32 互证），且 label-free、benchmark-free，换模型零标注可复现；

**动态剂量判决**：按 $s$ 加权分配剂量（$\beta_l=\beta\cdot w_l$，$\sum w=22$ 总剂量与平推严格对齐，`outputs/s_layer_weights.pt`；实现 `run_chair_hgai.py --layer-weights`）在 CHAIR 500 上 = **53.4/13.6/81.7/len117**——比 greedy（49.4）更差，远差于平推（30.0/6.4/73.5）。失败形态与 unimass α0.2 同族（recall 最高、len 最长：高响应层被过剂量，预算升高而约束稀释）。**动态剂量家族四连死**（statemod/homeo/atlas-weight/s-weight）——第四定律的剂量版：**慢性病配恒剂量；一切随层/随步变化的智能剂量都输平推**。

判决：剂量维持平推 L5-26 不变；$s$ 剖面进分析节作层段证据（不进公式）。

## 34. 空间频谱：视觉图的聚焦度可分性（50 图，2026-09-04，附录级）

对 24×24 patch 网格做 2D FFT（"对图片部分做频谱"的几何正确版），提取谱质心/高频功率占比/谱熵，在实体决定步比较 correct vs halluc（实现 `tools/logit_decomp/spatial_spectrum_stats.py`，数据 `outputs/spatial_spectrum_c{0-3}.jsonl`，373 实体；图 `assets/svd/spatial_spectrum.png`）：
...
| $F$ 图 | 谱质心 | +0.35 |
| $F$ 图 | 谱熵 | +0.31 |
| attention 图 | 谱质心 / 高频占比 | +0.26 / +0.28 |
| contribution 图 | 全部 | ≈0（无信号） |

**发现**：正确实体的 $F$ 图空间频率更高（更聚焦），幻觉的更弥散——"幻觉 = 看了但弥散"的空间频谱签名。效应中小（d≤0.45），进附录分析，不进主线。注：contribution 图（内容加权）无频谱差异——空间结构差异在 score 形状层，不在内容层。
